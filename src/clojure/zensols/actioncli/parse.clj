...
This extends the `clojure.tools.cli` package by allowing an *action* based
command line interface.  The first paramter of the command line is the action
and options for that respective action follow, much like `clone` would be an
action to the [Git](https://git-scm.com) command line tool and `--bare` would
be an argument to the `clone` action.

This namespace provides standard GNU conversion command line parsing, usage and
help formatting.  It includes:

* [[process-arguments]] Parse and process arguments.
* [[parse-arguments]] Parse command line arguments but do not execute.
* [[print-action-help]] Print the command help for a command line context.
* [[set-program-name]] The program name is used in the usage help output.

See the [project documentation](https://github.com/plandes/clj-actioncli#action-commands)
...
(def ^{:dynamic true :private true} *app-override* nil)
>>>>>>> merge
...
(defn- help-msg
  ...
  ([action-context actions action-key]
   (let [{:keys [print-help-fn usage-format-fn action-mode]} action-context
     ...
     (if usage-text (println usage-text))
     (->> (if action
            (action-help action)
            (->> (map #(get actions %) action-keys)
                 (map #(action-help % single-action-mode name-len))
...
=======
    ...
    (if errors
      (do
        (if print-errors?
          (->> [(error-message errors) (action-help action single-action-mode?)]
               (map println)
               doall))
        {:errors errors})
      (apply app (cons options arguments)))))
...
(defn parse-arguments
  "Parse command line arguments but do not execute.

  This is just [[process-arguments]] but it doesn't execute the `:app` function
  key bound function.  Instead it only returns the output of the parsed action,
  which is what the `:app`: function bound key returns.

  See [[process-arguments]] for parameter documentation."
  [action-context args]
  (letfn [(parse-app-fn [opts & args]
            {:opts opts
             :args args})]
    (binding [*dump-jvm-on-error* false
              *app-override* parse-app-fn]
      (process-arguments action-context args))))

(defn print-action-help
  "Print the command help for a command line context.
  ...
  (letfn [(parse-app-fn [opts & args]
    ...
    (binding [*dump-jvm-on-error* false
              *app-override* parse-app-fn]
      (process-arguments (merge action-context
                                ;; silence usage
                                {:usage-format-fn (fn [& _])})
                         []))))
